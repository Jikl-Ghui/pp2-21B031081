...
#Setting up Sounds
food_sound = pygame.mixer.Sound('./mp3/eat.ogg')
game_over_theme_sound = pygame.mixer.Sound('./mp3/game_over.ogg')
pygame.mixer.music.load('./mp3/Money.ogg')
bomb_sound = pygame.mixer.Sound('./mp3/bomb.ogg')
...
class Bomb:
    ...
    def draw(self):
        pygame.draw.rect(screen, (10, 85, 68), (self.x, self.y, 20, 20))
...
while running:
    ...
    if state == 'main menu' :
        ...
        if snake.eatB(food.x,food.y):
            bomb_sound.play()
            snake.is_remove=True
            bomb.gen()

        if snake.food(food.x, food.y) :
            food.gen()
        
        if snake.bomb(food.x,food.y):
            bomb.gen()
